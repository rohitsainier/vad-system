...
class PipelineConfig:
    ...
    # TTS — engine selector (kept for backwards compat; prefer "F5TTS")
    tts_engine: str = "F5TTS"

    # IndicF5 TTS settings
    indicf5_base_url: str = "http://localhost:8000"
    indicf5_reference_voice_key: str = ""
    indicf5_sample_rate: int = 16000
...
class VoiceChatPipeline:
    ...
    def initialize(self):
        ...
        # 4. Initialize TTS (IndicF5)
        self._tts_engine = TTSEngine(
            indicf5_base_url=self.config.indicf5_base_url,
            indicf5_reference_voice_key=self.config.indicf5_reference_voice_key,
            indicf5_sample_rate=self.config.indicf5_sample_rate,
            indicf5_output_format=self.config.indicf5_output_format,
            indicf5_timeout=self.config.indicf5_timeout,
            indicf5_seed=self.config.indicf5_seed,
            indicf5_normalize=self.config.indicf5_normalize,
        )
        ...
        active_voice = (
            self._tts_engine._indicf5_reference_voice_key or "(auto)"
        )
        logger.info(
            "Voice Chat Pipeline ready!",
            tts_voice=active_voice,
            tts_sample_rate=self.config.indicf5_sample_rate,
        )
    ...
    def process_speech(
        ...
        try:
            ...
            logger.info(
                "🎤 Processing speech",
                duration_ms=f"{audio_duration_ms:.0f}",
            )
            ...
            logger.info("Turn complete", latency=turn.latency_summary)
